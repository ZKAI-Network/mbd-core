...
def test_get_item_df(farcaster_casts_dataframe):
    ...
    for idx in range(len(farcaster_casts_dataframe)):
        ...
        assert pd.Timestamp(item[ITEM_CREATION_TIME_COLUMN]).tz_convert('UTC') == pd.Timestamp(cast["timestamp"]).tz_localize('UTC')
        assert pd.Timestamp(item[ITEM_UPDATE_TIME_COLUMN]).tz_convert('UTC') == pd.Timestamp(cast["timestamp"]).tz_localize('UTC')
        
        # Test root item derivation
        expected_root = "root" if pd.isna(cast["parent_hash"]) else "0x" + cast["root_parent_hash"]
        assert item[ROOT_ITEM_COLUMN] == expected_root, f"Root item doesn't match for record {idx}. Expected {expected_root}, got {item[ROOT_ITEM_COLUMN]}"
        ...
        expected_urls = list(dict.fromkeys(
            ...
            [embed["url"] for embed in cast["embeds"] if isinstance(embed, dict) and "url" in embed]
        ))
        assert item[EMBED_ITEMS_COLUMN] == expected_urls, f"URLs don't match for record {idx}"
        # Verify no duplicates
        assert len(item[EMBED_ITEMS_COLUMN]) == len(set(item[EMBED_ITEMS_COLUMN])), f"Duplicate URLs found in record {idx}"
        
        # Test user mentions
        assert isinstance(item[EMBED_USERS_COLUMN], list)
        assert item[EMBED_USERS_COLUMN] == [str(x) for x in cast["mentions"]], f"Mentions don't match for record {idx}"
...
def test_get_interaction_df(farcaster_casts_dataframe, farcaster_reactions_dataframe):
    ...
    for idx, cast in farcaster_casts_dataframe.iterrows():
        ...
        assert pd.Timestamp(post[TIME_COLUMN]).tz_convert('UTC') == pd.Timestamp(cast["timestamp"]).tz_localize('UTC')
        ...
        if pd.notna(cast["parent_hash"]):
            ...
            assert len(comment_interaction) == 1, f"Missing comment interaction for cast {idx}"
            ...
            assert pd.Timestamp(comment[TIME_COLUMN]).tz_convert('UTC') == pd.Timestamp(cast["timestamp"]).tz_localize('UTC')
    ...
    for idx, reaction in farcaster_reactions_dataframe.iterrows():
        ...
        assert pd.Timestamp(react[TIME_COLUMN]).tz_convert('UTC') == pd.Timestamp(reaction["timestamp"]).tz_localize('UTC')
...
def test_get_user_df(farcaster_users_dataframe):
    ...
    for idx, user in farcaster_users_dataframe.iterrows():
        ...
        assert pd.Timestamp(transformed[USER_CREATION_TIME_COLUMN]) == pd.Timestamp(user["created_at"]).tz_localize('UTC')
        assert pd.Timestamp(transformed[USER_UPDATE_TIME_COLUMN]) == pd.Timestamp(user["registered_at"])
